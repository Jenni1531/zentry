...
import zentry.back.api.business.controllers.TransactionsController;
...
@DisplayName("TransactionsController")
@SuppressWarnings("all")
class TransactionsControllerTest {

    @Autowired MockMvc mvc;
    @Autowired ObjectMapper json;
    @MockitoBean  TransactionsService service;

    private static final String BASE = "/api/business/transactions";
    private final UUID id = UUID.randomUUID();

    private TransactionsResponse sample() {
        return TransactionsResponse.builder().id(id).userId(1).amount(new BigDecimal("75.00")).build();
    }
...
class CreateTests {
        @Test void create_201() throws Exception {
            TransactionsRequest req = TransactionsRequest.builder().userId(1).amount(new BigDecimal("75.00")).build();
            ...
            mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(json.writeValueAsString(req)))
               .andExpect(status().isCreated()).andExpect(jsonPath("$.amount").value(75.00));
        }
    }
...
class UpdateTests {
        @Test void update_200() throws Exception {
            TransactionsRequest req = TransactionsRequest.builder().userId(1).amount(new BigDecimal("200.00")).build();
            when(service.update(eq(id), any())).thenReturn(TransactionsResponse.builder().id(id).userId(1).amount(new BigDecimal("200.00")).build());
        ...
        @Test void update_404() throws Exception {
            ...
            mvc.perform(put(BASE + "/" + u).contentType(MediaType.APPLICATION_JSON)
               .content(json.writeValueAsString(TransactionsRequest.builder().userId(1).amount(BigDecimal.TEN).build())))
