...
import zentry.back.api.business.controllers.PaymentMethodsController;
...
@DisplayName("PaymentMethodsController")
@SuppressWarnings("all")
class PaymentMethodsControllerTest {

    @Autowired MockMvc mvc;
    @Autowired ObjectMapper json;
    @MockitoBean  PaymentMethodsService service;

    private static final String BASE = "/api/business/payment-methods";
    private final UUID id = UUID.randomUUID();

    private PaymentMethodsResponse sample() {
        return PaymentMethodsResponse.builder().id(id).userId(1).metodo("CREDIT_CARD").build();
    }
...
class ListTests {
        @Test void list_200() throws Exception {
            ...
            mvc.perform(get(BASE)).andExpect(status().isOk())
               .andExpect(jsonPath("$.content[0].metodo").value("CREDIT_CARD"));
        }
    }
...
class GetByIdTests {
        @Test void getById_200() throws Exception {
            ...
            mvc.perform(get(BASE + "/" + id)).andExpect(status().isOk())
               .andExpect(jsonPath("$.metodo").value("CREDIT_CARD"));
        }
...
class CreateTests {
        @Test void create_201() throws Exception {
            PaymentMethodsRequest req = PaymentMethodsRequest.builder().userId(1).metodo("PAYPAL").build();
            when(service.create(any())).thenReturn(PaymentMethodsResponse.builder().id(UUID.randomUUID()).userId(1).metodo("PAYPAL").build());
            mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(json.writeValueAsString(req)))
               .andExpect(status().isCreated()).andExpect(jsonPath("$.metodo").value("PAYPAL"));
        }
        @Test void create_400_duplicate() throws Exception {
            ...
            mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON)
               .content(json.writeValueAsString(PaymentMethodsRequest.builder().userId(1).metodo("CREDIT_CARD").build())))
...
class UpdateTests {
        @Test void update_200() throws Exception {
            PaymentMethodsRequest req = PaymentMethodsRequest.builder().userId(1).metodo("DEBIT_CARD").build();
            when(service.update(eq(id), any())).thenReturn(PaymentMethodsResponse.builder().id(id).userId(1).metodo("DEBIT_CARD").build());
            mvc.perform(put(BASE + "/" + id).contentType(MediaType.APPLICATION_JSON).content(json.writeValueAsString(req)))
               .andExpect(status().isOk()).andExpect(jsonPath("$.metodo").value("DEBIT_CARD"));
        }
        @Test void update_404() throws Exception {
            ...
            mvc.perform(put(BASE + "/" + u).contentType(MediaType.APPLICATION_JSON)
               .content(json.writeValueAsString(PaymentMethodsRequest.builder().userId(1).metodo("X").build())))
