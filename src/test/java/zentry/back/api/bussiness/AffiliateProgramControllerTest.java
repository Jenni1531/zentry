...
import zentry.back.api.business.controllers.AffiliateProgramController;
...
@WebMvcTest(AffiliateProgramController.class)
@AutoConfigureMockMvc(addFilters = false)
@DisplayName("AffiliateProgramController")
@SuppressWarnings("all")
class AffiliateProgramControllerTest {

    @Autowired MockMvc mvc;
    @Autowired ObjectMapper json;
    @MockitoBean  AffiliateProgramService service;

    private static final String BASE = "/api/business/affiliate-program";
    private final UUID id = UUID.randomUUID();

    private AffiliateProgramResponse sample() {
        return AffiliateProgramResponse.builder().id(id).userId(1).build();
    }
...
class CreateTests {
        @Test void create_201() throws Exception {
            AffiliateProgramRequest req = AffiliateProgramRequest.builder().userId(2).build();
            when(service.create(any())).thenReturn(AffiliateProgramResponse.builder().id(UUID.randomUUID()).userId(2).build());
            mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(json.writeValueAsString(req)))
               .andExpect(status().isCreated()).andExpect(jsonPath("$.userId").value(2));
        }
        @Test void create_400_duplicate() throws Exception {
            ...
            mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON)
               .content(json.writeValueAsString(AffiliateProgramRequest.builder().userId(1).build())))
...
class UpdateTests {
        @Test void update_200() throws Exception {
            AffiliateProgramRequest req = AffiliateProgramRequest.builder().userId(3).build();
            when(service.update(eq(id), any())).thenReturn(AffiliateProgramResponse.builder().id(id).userId(3).build());
