...
class AdImpressionsControllerTest {
    ...
    @MockBean  AdImpressionsService service;

    private static final String BASE = "/api/business/ad-impressions";
    private final UUID id = UUID.randomUUID();

    private AdImpressionsResponse sample() {
        return AdImpressionsResponse.builder().id(id).campaignId(3).vistas(100).build();
    }
    ...
    @Nested @DisplayName("POST /") class CreateTests {
        @Test void create_201() throws Exception {
            AdImpressionsRequest req = AdImpressionsRequest.builder().campaignId(3).vistas(200).build();
            when(service.create(any())).thenReturn(AdImpressionsResponse.builder().id(UUID.randomUUID()).campaignId(3).vistas(200).build());
    ...
    @Nested @DisplayName("PUT /{id}") class UpdateTests {
        @Test void update_200() throws Exception {
            AdImpressionsRequest req = AdImpressionsRequest.builder().campaignId(3).vistas(500).build();
            when(service.update(eq(id), any())).thenReturn(AdImpressionsResponse.builder().id(id).campaignId(3).vistas(500).build());
        ...
        @Test void update_404() throws Exception {
            ...
            mvc.perform(put(BASE + "/" + u).contentType(MediaType.APPLICATION_JSON)
               .content(json.writeValueAsString(AdImpressionsRequest.builder().campaignId(3).vistas(1).build())))
