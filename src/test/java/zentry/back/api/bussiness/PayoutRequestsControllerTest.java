...
@WebMvcTest(PayoutRequestsController.class)
@DisplayName("PayoutRequestsController")
@SuppressWarnings("all")
class PayoutRequestsControllerTest {

    @Autowired MockMvc mvc;
    @Autowired ObjectMapper json;
    @MockitoBean  PayoutRequestsService service;

    private static final String BASE = "/api/business/payout-requests";
    private final UUID id = UUID.randomUUID();

    private PayoutRequestsResponse sample() {
        return PayoutRequestsResponse.builder().id(id).userId(1).amount(new BigDecimal("50.00")).build();
    }
...
class CreateTests {
        @Test void create_201() throws Exception {
            PayoutRequestsRequest req = PayoutRequestsRequest.builder().userId(1).amount(new BigDecimal("50.00")).build();
...
class UpdateTests {
        @Test void update_200() throws Exception {
            PayoutRequestsRequest req = PayoutRequestsRequest.builder().userId(1).amount(new BigDecimal("75.00")).build();
            when(service.update(eq(id), any())).thenReturn(PayoutRequestsResponse.builder().id(id).userId(1).amount(new BigDecimal("75.00")).build());
            mvc.perform(put(BASE + "/" + id).contentType(MediaType.APPLICATION_JSON).content(json.writeValueAsString(req)))
               .andExpect(status().isOk()).andExpect(jsonPath("$.amount").value(75.00));
        }
        @Test void update_404() throws Exception {
            ...
            mvc.perform(put(BASE + "/" + u).contentType(MediaType.APPLICATION_JSON)
               .content(json.writeValueAsString(PayoutRequestsRequest.builder().userId(1).amount(BigDecimal.TEN).build())))
